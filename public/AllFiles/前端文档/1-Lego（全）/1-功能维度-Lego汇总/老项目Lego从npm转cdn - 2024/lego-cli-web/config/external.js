...
function getDistVendorResources(key, isDevelopment) {
  ...
  if (isDevelopment) {
    ...
    scripts.push(
      path.join(__dirname, `../node_modules/${key}/dist/${key}.min.js`)
    );
  }
...
const externalConfigs = {
  ...
  'element-ui': {
    ...
    resources: () => ({
      scripts: [
        path.join(__dirname, '../node_modules/element-ui/lib/index.js'),
      ],
      styles: [
        path.join(
          __dirname,
...
module.exports = {
  ...
  chainWebpackExternals(config) {
    ...
    const { importScripts, importStyles, copyConfigs } = Object.keys(
      externalConfigs
    ).reduce(
      (result, key) => {
        ...
        scripts.forEach((script) => {
          ...
          result.importScripts.push(
            `<script src="${publicPath}/js/${distFileName}"></script>`
          );
        });
        ...
        styles.forEach((style) => {
          ...
          result.importStyles.push(
            `<link rel="stylesheet" href="${publicPath}/css/${distFileName}">`
          );
        });
    ...
    importScripts.push(
      `<script src="${publicPath}/napi/unpkg/global.js"></script>`
    );
    importScripts.push(
      `<script src="${publicPath}/napi/unpkg/lego-deps.js"></script>`
    );
    importScripts.push(
      `<script src="${publicPath}/napi/unpkg/lego.js"></script>`
    );
    importStyles.push(
      `<link rel="stylesheet" href="${publicPath}/napi/unpkg/lego.css">`
    );
